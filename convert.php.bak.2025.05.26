...
try {
    ...
    if ($type === 'engToNep') {
        ...
        $nepaliDate = LaravelNepaliDate::from($rawDate)->toNepaliDate(format: 'D, j F Y', locale: 'ne'); // Always return Nepali format
        ...
        $englishDate = LaravelNepaliDate::from($rawDate)->toEnglishDate(format: 'l, j F Y', locale: 'en'); // Always return English format
