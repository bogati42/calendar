...
class NepaliCalendar {
    ...
    private function generateNavigation($year, $month) {
        ...
        // Convert the 1st day of the current Nepali month to AD to get the first English month
        $ad_date_start = $this->convertBStoAD($year, $month, 1);
        $ad_month_name_start = date("F", mktime(0, 0, 0, $ad_date_start['month'], 1, $ad_date_start['year']));
        $ad_year_start = $ad_date_start['year'];

        // Convert the last day of the current Nepali month to AD to get the second English month
        $year_index = array_search($year, array_column($this->bs, 0));
        if ($year_index === false) {
            $year_index = array_search(2090, array_column($this->bs, 0)); // Fallback
        }
        $total_days_in_current_bs_month = $this->bs[$year_index][$month];
        $ad_date_end = $this->convertBStoAD($year, $month, $total_days_in_current_bs_month);
        $ad_month_name_end = date("F", mktime(0, 0, 0, $ad_date_end['month'], 1, $ad_date_end['year']));
        $ad_year_end = $ad_date_end['year'];

        $english_month_display = '';
        if ($ad_month_name_start === $ad_month_name_end) {
            $english_month_display = "$ad_month_name_start $ad_year_start AD";
        } else {
            // Display both months if they are different or span across years
            $english_month_display = "$ad_month_name_start / $ad_month_name_end $ad_year_start" . ($ad_year_start !== $ad_year_end ? "/$ad_year_end" : '') . " AD";
        }
        ...
        // Convert the BS year to Nepali numerals here
        $nepali_year = $this->convertToNepaliNumeral($year);

        // Display both BS (in Nepali numerals) and AD English months
        $nav .= "<h2>{$this->nep_months[$month]} $nepali_year <br>($english_month_display)</h2>";
     ...
     * Saturdays will now also use the 'holiday' class for styling.
    ...
    private function generateCalendarGrid($year, $month, $current_date, $holidays) {
        ...
        for ($day = 1; $day <= $total_days_in_month; $day++) {
            $class = 'day'; // Base class

            // Convert current BS day to AD for English date display and weekday calculation
            $ad_date_array = $this->convertBStoAD($year, $month, $day);

            // Determine if the current day is a Saturday
            $jd_current_day = gregoriantojd($ad_date_array['month'], $ad_date_array['day'], $ad_date_array['year']);
            $weekday_numeric_current_day = jddayofweek($jd_current_day, 0); // 0=Sun, ..., 6=Sat
            $is_saturday = ($weekday_numeric_current_day == 6);

            // Determine if the current day is 'today'
            $is_today = ($year === $current_date['year'] &&
                         $month === $current_date['month'] &&
                         $day === $current_date['day']);

            // Check if the current day is a listed holiday
            $is_listed_holiday = false;
            $holiday_name_for_display = ''; // Specific name for listed holidays
            foreach ($holidays as $holiday_item) { // Renamed loop var to avoid conflict
                if (!isset($holiday_item['bs_date']) || !is_array($holiday_item['bs_date'])) {
                    error_log("NepaliCalendar: Invalid holiday data structure for holiday: " . json_encode($holiday_item));
                    continue;
                }
                $h_year = (int)($holiday_item['bs_date']['year'] ?? 0);
                $h_month = (int)($holiday_item['bs_date']['month'] ?? 0);
                $h_day = (int)($holiday_item['bs_date']['day'] ?? 0);

                if ($h_year === $year && $h_month === $month && $h_day === $day) {
                    $is_listed_holiday = true;
                    $holiday_name_for_display = htmlspecialchars($holiday_item['name'] ?? 'Unknown Holiday');
                    error_log("NepaliCalendar: Listed Holiday matched: $holiday_name_for_display on $year-$month-$day");
                    break;
                }
            }

            // --- APPLY CLASSES ---
            // If it's a Saturday OR a listed holiday, apply the 'holiday' class
            if ($is_saturday || $is_listed_holiday) {
                $class .= ' holiday';
            }

            // Add 'today' class if it's the current date (can be combined with 'holiday')
            if ($is_today) {
                $class .= ' today';
            }
            // --- END APPLY CLASSES ---
            ...
            $english_day_display = date("d", mktime(0, 0, 0, $ad_date_array['month'], $ad_date_array['day'], $ad_date_array['year']));
            $escaped_holiday_name_for_js = addslashes($holiday_name_for_display); // Use specific holiday name for JS popups
            ...
            $grid .= "<div class='$class' " .
                         ...
                         "onmouseover='showDayDetailsHover(".
                             ...
                             $ad_date_array['month'].", ".
                             $ad_date_array['day'].", ".
                             "\"".$escaped_holiday_name_for_js."\", ". // Pass specific holiday name if any
                             "event)' " .
                         "onmouseout='hideDayDetailsHover()'>";
            $grid .= "<span class='nepali-day-number'>$nepali_day_numeral</span>";
            $grid .= "<span class='english-day-number'>$english_day_display</span>";

            // Only display the holiday name if it's a specifically listed holiday
            if ($holiday_name_for_display) {
                $grid .= "<div class='holiday-name' data-holiday-name='".htmlspecialchars($holiday_name_for_display, ENT_QUOTES, 'UTF-8')."'>$holiday_name_for_display</div>";
            }
            $grid .= "</div>";
        }
